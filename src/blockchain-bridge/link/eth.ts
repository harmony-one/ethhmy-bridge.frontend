import { ethLINKContract, ethLINKManagerContract, web3 } from '../ethSdk';
...
async function checkEthBalance(contract, addr) {
  return await ethLINKContract.methods.balanceOf(addr).call();
}
...
async function lockToken(userAddr, amount, sendTxCallback?) {
  ...
  let transaction = await ethLINKManagerContract.methods
...
async function unlockToken(userAddr, amount, receiptId) {
  ...
  const EthManagerJson = require('../out/LINKEthManager.json');
